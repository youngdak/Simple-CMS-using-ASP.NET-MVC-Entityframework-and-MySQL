(function($, window) {
    ...
    $.formUtils.addValidator({
        ...
        validatorFunction: function(value, $el, config, language, $form) {
            var conf = "", confInputName = $el.valAttr("confirm") || $el.attr("name") + "_confirmation", confInput = $form.find("input[name=\"" + confInputName + "\"]").eq(0);
            if (confInput) {
                conf = confInput.val();
            } else {
    ...
    var creditCards = { amex: [15, 15], diners_club: [14, 14], cjb: [16, 16], laser: [16, 19], visa: [16, 16], mastercard: [16, 16], maestro: [12, 19], discover: [16, 16] }, checkOnlyAmex = false, allowsAmex = false;
    $.formUtils.addValidator({
        name: "creditcard",
        validatorFunction: function(value, $el, config, language, $form) {
    ...
    var requestServer = function(serverURL, $element, val, conf, callback) {
            ...
            $.ajax({
                url: serverURL, type: "POST", cache: false, data: reqParams, dataType: "json",
                ...
                success: function(response) {
                    if (response.valid) {
                        ...
                        if (response.message)$element.attr(conf.validationErrorMsgAttribute, response.message);
                    }
                    if (!$element.valAttr("has-keyup-event")) {
                        $element.valAttr("has-keyup-event", "1").bind("keyup", function(evt) {
                            if (evt.keyCode != 9 && evt.keyCode != 16) {
                                $(this).valAttr("backend-valid", false).valAttr("backend-invalid", false);
                            }
                        });
                    }
                    callback();
        ...
        disableFormSubmit = function() { return false };
    $.formUtils.addValidator({
        name: "server",
        validatorFunction: function(val, $el, conf, lang, $form) {
            var backendValid = $el.valAttr("backend-valid"), backendInvalid = $el.valAttr("backend-invalid"), serverURL = document.location.href;
            if ($el.valAttr("url")) {
            ...
            else if (backendInvalid)return false;
            else if ($.formUtils.eventType == "keyup")return null;
