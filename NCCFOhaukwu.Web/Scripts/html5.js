(function($, window) {
    "use strict";
    var SUPPORTS_PLACEHOLDER = "placeholder" in document.createElement("INPUT"), SUPPORTS_DATALIST = "options" in document.createElement("DATALIST");
    $(window).bind("validatorsLoaded formValidationSetup", function(evt, $form) {
        ...
        $form.each(function() {
            var $f = $(this), $formInputs = $f.find("input, textarea, select"), foundHtml5Rule = false;
            $formInputs.each(function() {
                var validation = [], $input = $(this), isRequired = $input.attr("required"), attrs = {};
                ...
                case"number":
                    ...
                    var max = $input.attr("max"), min = $input.attr("min");
                    if (min || max) {
                        if (!min)min = 0;
                        if (!max)max = 9007199254740992;
                ...
                if (validation.length) {
                    ...
                    $.each(attrs, function(attrName, attrVal) { $input.attr(attrName, attrVal) });
                }
            });
            if (foundHtml5Rule) {
