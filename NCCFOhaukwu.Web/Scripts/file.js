(function($, window) {
    ...
    $.formUtils.addValidator({
        ...
        validatorFunction: function(str, $input, conf, language) {
            if (SUPPORTS_FILE_READER) {
                var valid = true, files = $input.get(0).files || [], mime = "", allowedTypes = _getTypes($input);
                if (files.length) {
    ...
    $.formUtils.addValidator({
        ...
        validatorFunction: function(value, $input, conf, language) {
            var valid = true, _this = this, allowedTypes = _getTypes($input);
            $.each($input.get(0).files || [value], function(i, file) {
                var val = typeof file == "string" ? file : file.value || file.fileName || file.name, ext = val.substr(val.lastIndexOf(".") + 1);
    ...
    $.formUtils.addValidator({
        ...
        validatorFunction: function(val, $input, conf, language) {
            ...
            var maxBytes = $.formUtils.convertSizeNameToBytes(maxSize), valid = true;
    ...
    $(window).one("validatorsLoaded formValidationSetup", function(evt, $form) {
        ...
        $inputs.filter("*[data-validation]").bind("change", function() { $(this).removeClass("error").parent().find(".form-error").remove() });
    });
})(jQuery, window);